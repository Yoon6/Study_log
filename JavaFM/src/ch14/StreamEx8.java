...
public class StreamEx8 {
    public static void main(String[] args) {
        ...
        Map<Integer, List<Student2>> stuByBan = Stream.of(stuArr).collect(
        ...
        Map<Student2.Level, List<Student2>> stuByLevel = Stream.of(stuArr).collect(
                groupingBy(s->{
                    if(s.getTotalScore()>=200) return Student2.Level.HIGH;
                    else if(s.getTotalScore()>=100) return Student2.Level.MID;
        ...
        Map<Student2.Level, Long> stuCntByLevel = Stream.of(stuArr).collect(
                groupingBy(s->{
                    if(s.getTotalScore()>=200) return Student2.Level.HIGH;
                    else if(s.getTotalScore()>=100) return Student2.Level.MID;
                    else return Student2.Level.LOW;
        ...
        Map<Integer, Map<Integer, Student2>> topStuByHakAndBan =
                ...
                .collect(groupingBy(Student2::getHak, groupingBy(Student2::getBan,
                        collectingAndThen(
                                maxBy(comparingInt(Student2::getTotalScore)),
                                Optional::get
                        ))));
        ...
        Map<String, Set<Student2.Level>> stuByScoreGroup = Stream.of(stuArr).collect(
                groupingBy(s-> s.getHak()+"-"+s.getBan(),mapping(s->{
                    if(s.getTotalScore()>=200) return Student2.Level.HIGH;
                    else if(s.getTotalScore()>=100) return Student2.Level.MID;
                    else return Student2.Level.LOW;
                }, toSet())
